...
<head>
	...
	<title>OTF Los Gatos: Class Layout</title>
</head>
<body>
<div class="container">
	<div class="row">
...
$sql = "SELECT * FROM LosGatos WHERE Day = '$today' AND Hour = '$time';";
...
if ($resultCheck > 0) {
	...
	if(empty($row['Floor1']) && empty($row['Floor2']) && empty($row['Floor3']) && empty($row['Floor4']) && empty($row['Floor5']) && empty($row['Floor6']) && empty($row['Floor7']) && empty($row['Floor8']) && empty($row['Floor9']) && empty($row['Floor10']) && empty($row['Floor11']) && empty($row['Floor12']) && empty($row['Floor13'])){
		...
		echo "</tr>";
		echo "<tr>";
			echo "<td class='last' id='t13'>13: &nbsp;" . $row['Tread13'] . "</td>"; 
			echo "<td class='last' id='r13'>13: &nbsp;" . $row['Rower13'] . "</td>";
		echo "</tr>";
		echo "</table>";
		echo "</div>";
		echo "<div id='container'>";
		echo "<span id='time'>" . $time . "</span>";
		echo "<button class='rosterButton'><a href='https://adamgolab.com/losgatos/otf.php'>Go Back</a></button>";
		echo "<button class='rosterButton' id='2GRotate' onclick='twoGroupRotate()'>Rotate</button>";
		echo "<button class='rosterButton btn btn-default' style='display: none' id='twoG' onclick=\"document.getElementById('templateRoster').src='../losgatos/2G/" . $todayDay . ".pdf'; switchVisible();\">Switch to 2G</button>";
		echo "<button class='rosterButton btn btn-default' id='threeG' onclick=\"document.getElementById('templateRoster').src='../losgatos/3G/" . $todayDay . ".pdf'; switchVisible();\">Switch to 3G</button>";
		$day = date("Ymj");
		$dayName = date('l', $day);
		if(date($dayName === 'Wednesday')) {
			echo "<button class='rosterButton btn btn-default' id='fourty' onclick=\"document.getElementById('templateRoster').src='../losgatos/45/" . $todayDay . ".pdf'\">Switch to 45</button>";
		}
		echo "<span id='time'>" . $count . " out of 26</span>";
		echo "</div>";
	} else {
		for($i = 1; $i <= 13; $i++) {
			$check = "Tread".$i;
		...
		echo "<tr>";
			echo "<td class='last' id='t13'>13: &nbsp;" . $row['Tread13'] . "</td>"; 
			echo "<td class='last' id='r13'>13: &nbsp;" . $row['Rower13'] . "</td>";
			echo "<td class='last' id='f13'>13: &nbsp;" . $row['Floor13'] . "</td>"; 
		echo "</tr>";
		echo "</table>";
		echo "</div>";
		echo "<div id='container'>";
		...
		echo "<button class='rosterButton btn btn-default' id='twoG' onclick=\"document.getElementById('templateRoster').src='../losgatos/2G/" . $todayDay . ".pdf'; switchVisible();\">Switch to 2G</button>";
		echo "<button class='rosterButton btn btn-default' style='display: none' id='threeG' onclick=\"document.getElementById('templateRoster').src='../losgatos/3G/" . $todayDay . ".pdf'; switchVisible();\">Switch to 3G</button>";
		$day = date("Ymj");
		$dayName = date('l', $day);
		if(date($dayName === 'Wednesday')) {
			echo "<button class='rosterButton btn btn-default' id='fourty' onclick=\"document.getElementById('templateRoster').src='../losgatos/45/" . $todayDay . ".pdf'\">Switch to 45</button>";
		}
		echo "<span id='time'>" . $count . " out of 36</span>";
		echo "</div>";
	}
}
}
...
<script>
	...
	if(empty($row['Floor1']) && empty($row['Floor2']) && empty($row['Floor3']) && empty($row['Floor4']) && empty($row['Floor5']) && empty($row['Floor6']) && empty($row['Floor7']) && empty($row['Floor8']) && empty($row['Floor9']) && empty($row['Floor10']) && empty($row['Floor11']) && empty($row['Floor12']) && empty($row['Floor13'])){
		echo "</div>";
		echo "<div>";
		echo "<iframe id='templateRoster' src='../losgatos/2G/" . $todayDay . ".pdf' width='850' height='450'></iframe>";
		}
		else {
		echo "</div>";
		echo "<div>";
		echo "<iframe id='templateRoster' src='../losgatos/3G/" . $todayDay . ".pdf' width='850' height='450'></iframe>";
		}
		echo "</div>";
		echo "";
	}
